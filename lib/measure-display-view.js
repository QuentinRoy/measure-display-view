import { Directions, noOp } from "./utils.js"
import DragControler from "./drag-controller.js"

function template({ rulerOrientation }) {
  const orientationClassName = `${rulerOrientation}-rulers`
  return `
...
export default function MeasureDisplayView({
  ...
  node.innerHTML = template({ rulerOrientation })
  ...
  function onDragStart({ direction, position, target }) {
    node.style.cursor =
      direction === Directions.vertical ? "ns-resize" : "ew-resize"
  ...
  const result = { onChange, getRatio, setRatio, getDisplayDimensions, remove }
